...
class Interpreter {
  ...
  /// Long-lived execution session — all cells share the same dylib.
#ifndef __EMSCRIPTEN__
  std::unique_ptr<SwiftJIT> JIT;
#else
  std::unique_ptr<SwiftWasmExecutor> WasmExec;
#endif
...
public:
  ...
  /// Returns true if the execution session was successfully initialised.
  bool isReady() const {
#ifndef __EMSCRIPTEN__
    return (bool)JIT;
#else
    return (bool)WasmExec;
#endif
  }
