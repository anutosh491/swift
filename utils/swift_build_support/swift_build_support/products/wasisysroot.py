...
class WASISysroot(product.Product):
    ...
    def _build_runtimes(self, runtimes_build_dir, target_triple, sysroot_install_path,
        ...
        wasmsysroothelpers.build_runtimes(
            args=self.args,
            toolchain=self.toolchain,
            runtimes_source_dir=os.path.join(
                os.path.dirname(self.source_dir), 'llvm-project', 'runtimes'),
            runtimes_build_dir=runtimes_build_dir,
            install_path=WASISysroot.sysroot_install_path(
                build_root, target_triple),
            enable_threads=(cmake_has_threads == 'TRUE'),
            libdir_suffix='/' + target_triple,
            append_platform_cmake_options=self._wasi_cmake_options_configurator(
                sysroot_install_path, target_triple,
    ...
    def _build_compiler_rt(self, compiler_rt_build_dir, target_triple, compiler_rt_os_dir,
        ...
        wasmsysroothelpers.build_compiler_rt(
            ...
            compiler_rt_build_dir=compiler_rt_build_dir,
            install_path=WASISysroot.resource_dir_install_path(
                build_root, target_triple),
            compiler_rt_os_dir=compiler_rt_os_dir,
            append_platform_cmake_options=self._wasi_cmake_options_configurator(
                sysroot_install_path, target_triple,
    ...
    def _wasi_cmake_options_configurator(self, sysroot_build_path, target_triple,
                                     cc_path, cxx_path, ar_path, ranlib_path,
                                     c_flags, cxx_flags):
    ...
    def _build_compiler_rt_builtins(self, compiler_rt_build_dir, target_triple,
        ...
        self._wasi_cmake_options_configurator(
            sysroot_install_path, target_triple,
            cc_path, cxx_path, ar_path, ranlib_path,
            c_flags, cxx_flags)(compiler_rt.cmake_options)
